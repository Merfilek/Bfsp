...
// Liquid Glass is a whole-presentation theme (currentPresentationTheme, js/themes.js),
// not a per-element setting, so it's applied here at the bottom of the shape/image
// content renderers rather than stored on each element -- flipping the theme re-renders
// every shape and image at once, the same way a real theme switch works.
const LIQUID_GLASS_MIN_RADIUS = 4;
const LIQUID_GLASS_MAX_RADIUS = 22;

function liquidGlassCornerRadius(element) {
  return Math.max(LIQUID_GLASS_MIN_RADIUS, Math.min(LIQUID_GLASS_MAX_RADIUS, element.width / 6, element.height / 6));
}

function appendBoxShadow(el, shadow) {
  el.style.boxShadow = el.style.boxShadow ? el.style.boxShadow + ", " + shadow : shadow;
}

function applyLiquidGlassShapeStyle(el, element) {
  if (currentPresentationTheme !== "liquidGlass") return;

  el.style.backgroundColor = hexToRgba(element.fillColor || "#ffffff", 0.28);
  const sheen = "linear-gradient(135deg, rgba(255,255,255,0.45), rgba(255,255,255,0.08))";
  el.style.backgroundImage = el.style.backgroundImage ? sheen + ", " + el.style.backgroundImage : sheen;
  el.style.backdropFilter = "blur(18px) saturate(180%)";
  el.style.webkitBackdropFilter = "blur(18px) saturate(180%)";
  el.style.border = "1px solid rgba(255,255,255,0.45)";

  if (element.shapeType === "rectangle" && !element.borderRadius) {
    el.style.borderRadius = liquidGlassCornerRadius(element) + "px";
  }

  appendBoxShadow(el, "0 8px 24px rgba(0,0,0,0.18), inset 0 1px 0 rgba(255,255,255,0.55)");
}

function applyLiquidGlassImageStyle(el, element) {
  if (currentPresentationTheme !== "liquidGlass") return;
  ...
  appendBoxShadow(el, "0 10px 28px rgba(0,0,0,0.22)");
  ...
  sheen.style.backgroundImage = "linear-gradient(135deg, rgba(255,255,255,0.4), rgba(255,255,255,0.05) 45%, rgba(255,255,255,0) 60%)";
...
function renderImageContent(el, element) {
  ...
  applyLiquidGlassImageStyle(el, element);
}
...
function renderShapeContent(el, element) {
  ...
  applyLiquidGlassShapeStyle(el, element);
}
